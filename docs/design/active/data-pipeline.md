...
- **社区共识度取首选项的原始百分比，不做归一化。** 源站的 distribution
...
| 缺陷 | 成因 | 规模 | 修法 | 位置 |
|---|---|---|---|---|
| 缩写句点被吞：`VPC. Create → VPCreate` | 源站抓取把正文里「字母. 」当选项标签剥掉；末字母落 A–F 的缩写中招 | 34 题 | 缩写表 `{VP→VPC, AL→ALB, NL→NLB, MF→MFA}` × 后接大写动词 | `banks/aws-sap-c02/parse.py`（源格式特有） |
