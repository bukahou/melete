...
| P8 | **中日双语**：界面 i18n + AWS 题库日文内容 | 🔄 界面已上 dev；SAA 题面翻译进行中 |
...
## P8 · 中日双语 🔄（2026-09-08 起）

**目标（2026-09-08 用户明确）**：**melete 这个应用本身的所有操作面都要能用中文或日文。**
一个日本人从登录、选题库、刷题、看统计到改设置，全程日文，不该在任何一处撞见中文。

⚠️ 与「内容多语言」不是一回事，这里踩过一次：
| | 做 | 不做 |
|---|---|---|
| 应用界面 | 中/日 ✅ | |
| **AWS 两库正文** | **要日文**（现在只有中文） | |
...
- [x] 后端按 `Accept-Language` 供稿（`internal/httplocale`，标准内容协商 + 白名单 + `Vary`）
- [x] `question_i18n` / `choice_i18n`（dev 已建；⚠️ prod TiDB 见下方「合 main 前的闸」）
- [x] 前端 next-intl **不带路由**：语言是用户偏好不是资源标识，同一道题不该有两个 URL
- [x] 优先级 **手动选择(cookie) > 浏览器 Accept-Language > zh**；实测 6 种输入全部符合
- [x] ⭐ 缺译文**显示原文并标注**，⛔ 不静默回退 —— 与「答案主张并列展示」同一条哲学
- [x] `tagTypeLabel`/`tagName` 的 locale **去掉默认值** ⇒ 漏传 = 编译失败（默认 "zh" 会静默显示中文，是 i18n 最难查的一类 bug）
- [x] 翻译段 `pipeline/core/translate.py` + 封闭术语表 + 假名哨兵

### 进行中 —— SAA-C03 题面日文（派给「解析」会话，2026-09-08）

只翻**题干+选项**（每题约 400 字）；解析（每题约 2000 字，占全库 83%）单独一趟后补 ——
缺了只是界面回退中文并标注，⛔ 不阻塞「日本人能不能做题」。

⭐ **假名哨兵**是这一段最重要的校验：中文里没有假名，所以「忘了翻、把中文原样抄过来」
必然被抓。⚠️ 刻意不用「译文 ≠ 原文」做判据 —— 那与被测者同源，改几个字就能骗过；
假名骗不过去。（同一条教训：「验证工具与被验证者共享同一盲区时，验证必然通过」。）

⚠️ 实际用一次就抓到三个自造缺陷，都是不用就发现不了的：
① 术语表把**正确**的译文判成错（「应用负载均衡器」在 AWS 日文里是 Application Load Balancer）
② 只翻题面那趟，解析里的术语全报假警 —— **误报比漏报更伤**，它训练人忽略检查，闸门就等于不存在
③ `status` 说下一片是 0026、`next` 给的是 0001，**两边都不报错** ⇒ 跨天做的人会做错整片

### ⭐ 跨会话协作里学到的两条（会话一结束就会丢，写在这里）

**① 保守的统计会【系统性】低估缺陷规模。**

「解析」会话两次报规模都偏低（21 vs 3 是量错了单位、39 vs 104 是漏了样本），
方向一致 —— 它自己复现出根因是两行「为了降噪」的过滤：
`if len(t) < 3: continue` 把「实例/队列/日志」这些 2 字词条一个都没数，
⚠️ 而「队列」正是最该数的那个（复合词被切断后接管的就是它）。
⇒ **一条为了降噪的过滤，把最关键的样本过滤掉了。**
后果不是数字难看，是**让「值不值得修」这个判断失真**。

⇒ 报规模时先说清四项：**单位**（处/条/题）· **去重**（唯一位置/出现次数）·
  **过滤**（有没有排除条件）· **条件数**（单条件 / 闸门实际的多条件）。

**② 报风险时要顺手量一下，⛔ 别把量化整个丢给对方。**

「解析」提「折叠空白可能破坏 keep 列表」时只写了一句备注、没量。
实测是 **52 个含空格的服务名里 28 个会失效** —— 一个可能很严重的风险被报成了轻描淡写。
⭐ 量它只要三行。**该测的没测完就报，与上面那条是同一个毛病。**

（⚠️ 我这边也犯过同型的：`shift_jis` 判据是机械的、可复现的，
但它测的不是要测的那件事 —— **机械可执行 ≠ 测对了东西**。）

### 待办

- [ ] SAA-C03 题面 41 片（进行中）→ 解析 41 片（后一趟）
- [ ] SAP-C02：⭐ **从 `enriched.json` 里的英文原文译**，⛔ 不从中文译文再译一层（避免 en→zh→ja 二次失真）
- [ ] `load.py` 把译文写进 `question_i18n`/`choice_i18n`，⛔ 拆掉 `tr.get("stem") or q["stem"]` 那条**烧死译文**的老路
- [ ] 标签显示名：`concept` 1075 个是 kebab slug（`auto-scaling`），**中文界面也看不懂**；`domain` 只有 en/zh 无 ja。⚠️ 边界待定：标签算「应用操作面」还是「题库内容」
- [ ] `ipa-ip` 的 domain i18n 键放错 —— 日文塞进了 `en` 键（`{"en": "ストラテジ系"}`）
- [ ] 1075 个 concept 标签 `bank_id = 0` 而非 NULL —— 悬空引用。目前无故障，
      但下一个按 `bank_id IS NULL` 查全局标签的人会让它们集体消失。
      与本项目在账号 id 上吃过的「零值不得当成合法身份」是同一形状
- [ ] 界面要不要加英文（英文/韩文浏览器现在落到 zh）—— 待用户裁
