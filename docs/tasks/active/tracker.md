...
| P7 | 第三个题库：IPA IT パスポート（日文 + 扫描版） | 🔄 令和8年度已转写，富化待开工 |
...
现有两个题库是第三方汇编，AWS 协议明确禁止披露考试内容 ⇒ 仓库必须永久 private，
⛔ **不能拿去社内推广**。而 IPA **官方主动公开**历年问题与解答供人学习 ——
社内推广完全正当，「把 backend/web/pipeline-core 拆出去开源」这条路也随之打开。
用户所在公司要求这张证书，可在社内推广。
...
- [ ] **决策：`topic` 轴用官方シラバス「中分類」23 个（封闭词表）** ← 待用户拍板
...
- [ ] 写 `enrich_spec.json` + 富化（**解析用日文**，用户已定）
...
- [ ] **限流键只有 `clientIP`**（`httpauth/ratelimit.go`）：换 IP 轮换即可绕开，
      分布式来源打同一账号是零防护。⚠️ 但账号维度的处置**不能简单加上去** ——
      全局按账号做拒绝/延迟会变成 DoS 面（攻击者用自己的失败尝试消耗受害者配额）
      且是预言机（一次探测即读出该账号近期有无失败）。
      定稿方向：拒绝按 `(来源, 账号)` 且计数与账号是否存在无关；全局按账号只用于告警。
...
| 仓库可见性 | 必须永久 private，理由见根目录 CLAUDE.md。⚠️ **IPA 题库是唯一版权干净的**，将来拆分开源以它为据 |
