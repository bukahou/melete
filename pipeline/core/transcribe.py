...
def cmd_next(bank: str, count: int) -> None:
    ...
    # ⚠️ 题都转完了就收工，哪怕还有页没分到片。
    #
    # 卷子末尾往往是附录而不是题（令和8年度 p50-56 是「表計算ソフトの機能・用語」，
    # 官方给的函数与术语定义）。原来只按【页】判断待办，于是 100/100 之后
    # next 仍在索要那一片 —— 而「已转写 100/100」就打印在上一行，
    # 信息在眼前却没用上。
    #
    # 答案 PDF 是「一共有多少题」的权威来源：它说完了就是完了，
    # 剩下的页按定义不是题。⛔ 不要再引入「这几页没有题」之类的人工标记 ——
    # 那又是一份要维护的先验。
    if done_nos and done_nos == set(answers):
        print("✓ 与答案 PDF 完全对账，题已转完（剩余页面是附录一类，不含题）")
        print("  下一步：transcribe.py check → merge")
        return
...
⚠️ 规则：
  ...
  · ⛔⛔ **题干里被下划线强调的否定，必须保留**，写成 **必要がない** 这样的加粗：
    「認証に必要な情報の変更を定期的に行う〈必要がない〉ものだけを全て挙げたものはどれか」
    —— 丢掉那个强调，整道题的意思就【反了】，而且「正确答案」会看起来是错的。
    ⚠️ 这比漏抄一个表格严重得多：漏表格一眼能看出缺东西，
    否定反转看起来完全正常，只有做题的人会莫名其妙。
    （2016h28h 問90 实例；令和8 那套恰好没有，所以第一轮没暴露出来。）
  · ⚠️ 用文字【替代】了原图的某种表达时（下划线实线/虚线、颜色、箭头粗细…），
    在替代处补一句方括号说明，例如 "［原図では実線＝主キー，破線＝外部キー］"。
    ⛔ 否则题干里那句「実線は主キーを表す」会指向一个已经不存在的东西 ——
    信息没丢，但读者会对着一段找不到实线的文字发愣。
...
def cmd_write(bank: str, src: Path) -> None:
    ...
    #
    # ⭐ 备选方案是「保持覆盖 + 题数偏少时告警」，没选它，理由由解析会话给得最准：
    #   check 只报「答案里有但还没转写」，而这条信息在【还没写到那一片】与
    #   【写了但被覆盖丢了】两种情况下**长得一模一样**。要让告警区分两者，
    #   就得再引入一个「这片应该有多少题」的先验 —— 而每片题数随版面浮动，
    #   那个先验本身不存在。让缺陷写不出来，比检测它省事也可靠。
