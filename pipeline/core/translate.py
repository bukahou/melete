...
#
# ⚠️ 2026-09-08 清过一次杂质：靠人眼挑必然混进日语汉字，我第一版就混进了 6 个
# （个 从 价 凭 网 阻）。「解析」撞上的是「阻」——阻止/阻害是常用漢字，
# 而闸门把正确的「アクセスを阻止する」判成了中文原文。
#
# ⭐ 准入判据必须机械可执行，⛔ 不能靠印象：
#     用 Shift-JIS 试编码 —— 它覆盖的正是 JIS X 0208 日本汉字集，编得进去就不该入表。
#         '阻'.encode('shift_jis')  → OK   ⇒ 是日语汉字，剔除
#         '亚'.encode('shift_jis')  → 抛错 ⇒ 非日语汉字，可以留
# ⚠️ JIS X 0208 比「现代日语实际会用的字」宽（含 从/价/凭/网 这类罕见异体），
#    所以按它筛会剔掉一些其实很安全的字。**这个方向的保守是有意的**：
#    误伤正确译文会逼译者改写内容（已实证两次），而漏网只是让中文留在原地，
#    还有「汉字≥6 需有假名」那条兜底。实测代价：多漏 3 条（0.06%）。
#
# 加字前先跑：python3 -c "'X'.encode('shift_jis')" —— 不抛错就别加。
SIMPLIFIED_ONLY = frozenset("亚们为义乐习书买产亲仓优传伤备复头实宁审层岁币帮师带库应废开张录惊户执扩报拟损时显术权业东严临举仅计订认讨让训议记讲论设访证评识诉词试详语误说请读课调谈谁谢贝财责账货质购贷费资车转轮软轻输达过运进连选递适远违边还这钟银错键镜锁铁问间闭关阅险隐项顺须领频题顾预页风飞马驱验组织经结给络统继线级纪约纳纸终维缓编缩电长门队阶动势发变处标样检构树桥极测济满灭现环监盘确码离种积稳竞笔简类罗职联胜脑节药获营蓝补见观规视览觉单储启击邻务鲁历纵览")
